...
export const multiplayerQuizSubmissions = pgTable(
  'multiplayer_quiz_submissions',
  {
...
export const userUsageData = pgTable(
  'user_usage_data',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    userId: uuid('user_id').notNull(),
...
export const userResponses = pgTable(
  ...
  (table) => [
    ...
    pgPolicy('Users can insert their own responses', {
      as: 'permissive',
      for: 'insert',
      to: ['authenticated'],
    }),
...
export const questions = pgTable(
  ...
  (table) => [
    foreignKey({
      ...
      .onUpdate('cascade')
      .onDelete('cascade'),
    pgPolicy('Anyone can view questions', {
      as: 'permissive',
      for: 'select',
      to: ['authenticated'],
      using: sql`true`,
    }),
    pgPolicy('Quiz owners can manage questions', {
      as: 'permissive',
      for: 'all',
      to: ['authenticated'],
    }),
...
export const bookmarks = pgTable(
  ...
  (table) => [
    ...
    pgPolicy('Users can view their own bookmarks', {
      as: 'permissive',
      for: 'select',
      to: ['authenticated'],
      using: sql`(( SELECT auth.uid() AS uid) = user_id)`,
    }),
    pgPolicy('Users can manage their own bookmarks', {
      as: 'permissive',
      for: 'insert',
      to: ['authenticated'],
    }),
    pgPolicy('Users can delete their own bookmarks', {
      as: 'permissive',
      for: 'delete',
      to: ['authenticated'],
    }),
...
export const quizzes = pgTable(
  ...
  (table) => [
    ...
    foreignKey({
      ...
      .onUpdate('cascade')
      .onDelete('cascade'),
    pgPolicy('Users can create quizzes', {
      as: 'permissive',
      for: 'insert',
      to: ['authenticated'],
      withCheck: sql`(( SELECT auth.uid() AS uid) = user_id)`,
    }),
    pgPolicy('Users can update their own quizzes', {
      as: 'permissive',
      for: 'update',
      to: ['authenticated'],
    }),
    pgPolicy('Users can delete their own quizzes', {
      as: 'permissive',
      for: 'delete',
      to: ['authenticated'],
    }),
...
export const rooms = pgTable(
  ...
  (table) => [
    ...
    pgPolicy('Anyone can view rooms', {
      as: 'permissive',
      for: 'select',
      to: ['authenticated'],
      using: sql`true`,
    ...
    pgPolicy('Room hosts can update rooms', {
      as: 'permissive',
      for: 'update',
      to: ['authenticated'],
    }),
